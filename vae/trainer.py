...
class Trainer:
    ...
    def __init__(self, model, train_loader, test_loader, optimizer,
                 loss_function, channels, image_size=32, device='cpu'):
        self.model = model
    ...
    def plot_generated(self):
        x = vutils.make_grid(data[:5, :, :, :], normalize=True, scale_each=True)
        self.writer.add_image('img/real', x)

        x = vutils.make_grid(recon_x.reshape(-1, self.channels, self.image_size, self.image_size)[:5, :, :, :],
                             normalize=True, scale_each=True)
        self.writer.add_image('img/rec', x)
