...
from homework.trainer import DCGANTrainer
...
def main():
    ...
    dataset = datasets.MNIST(root=config.data_root, download=True,
                               transform=transform)
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=config.batch_size, shuffle=True,
                                             num_workers=4, pin_memory=True)

    discriminator, generator = DCDiscriminator(config.image_size, 1), DCGenerator(config.image_size, 1, 100)
